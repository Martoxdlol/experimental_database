...
impl SecondaryIndex {
    ...
    pub async fn insert_entry(&self, encoded_key: &[u8]) -> std::io::Result<()> {
        self.btree.insert(encoded_key, &[]).await
    }
...
async fn verify(
    ...
    ts: Ts,
    read_ts: Ts,
) -> std::io::Result<Option<(DocId, Ts)>> {
    match primary.get_version_ts(doc_id, read_ts).await? {
...
mod tests {
    ...
    async fn version_resolution_in_scan() {
        ...
        sec.insert_entry(&key5).await.unwrap();
        sec.insert_entry(&key10).await.unwrap();
    ...
    async fn tombstoned_doc() {
        ...
        let d = doc(1);
        primary.insert_version(&d, 5, Some(b"body")).await.unwrap();
        primary.insert_version(&d, 10, None).await.unwrap(); // tombstone
